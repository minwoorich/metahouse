...
@Service
public class PortfolioAttachFileUploadLogicService {
	//파일업로드를 수행하는 메소드 - 업로드된 파일의 정보를 BoardFileDTO로 변환해서 리턴(1개인 경우)
	//여러개인 경우 BoardFileDTO를 List에 담아서 리턴
	public List<PortfolioAttachFileDTO> uploadFiles(List<MultipartFile> multipartFiles, String path) throws IllegalStateException, IOException {
		...
		for(MultipartFile multipartFile : multipartFiles) {
			...
			filedtolist.add(new PortfolioAttachFileDTO(null, null, multipartFile.getOriginalFilename(), storeFilename, count+""));
			count++;
		}
		return filedtolist;
	}
