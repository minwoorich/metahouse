...
@Service
public class AssetReviewFileUploadService {
	public List<AssetReviewJpaDto.AssetReviewContents> uploadFiles(List<MultipartFile> multipartFiles, String path)
			throws IllegalStateException, IOException {
		
		List<AssetReviewJpaDto.AssetReviewContents> contentsList = new ArrayList<AssetReviewJpaDto.AssetReviewContents>();
		int count = 1;
		for (MultipartFile multipartFile : multipartFiles) {
			// uploadFile() 호출해서 사용
			String storeFilename = uploadFile(multipartFile,path);
			//AssetContentsEntity 데이터 옮겨담기
			
			contentsList.add(AssetReviewJpaDto.AssetReviewContents
					.builder()
