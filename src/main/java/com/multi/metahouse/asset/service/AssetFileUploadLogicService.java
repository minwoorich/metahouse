...
public class AssetFileUploadLogicService {
	public List<AssetDetailImgDTO> uploadFiles(List<MultipartFile> multipartFiles, String path)
			throws IllegalStateException, IOException {
		List<AssetDetailImgDTO> contentsList = new ArrayList<AssetDetailImgDTO>();
		int count = 1;
		for (MultipartFile multipartFile : multipartFiles) {
			// uploadFile() 호출해서 사용
			String storeFilename = uploadFile(multipartFile,path);
			//ProjectContentsEntity 데이터 옮겨담기
			AssetDetailImgDTO dto = new AssetDetailImgDTO();
			dto.setAsset_detail_img_store_filename(storeFilename);
			dto.setAsset_fileno(count+"");
